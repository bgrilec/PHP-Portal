<?php 

require_once "functions.php";
require_once "dbConnection.php";

$id_korisnika = dataFilter('id_korisnika', 'i');
$uloga = dataFilter('uloga', 'i');
$ime = dataFilter('ime', 's');
$prezime = dataFilter('prezime', 's');
$adresa = dataFilter('adresa', 's');
$mjesto = dataFilter('mjesto', 's');
$drzava = dataFilter('drzava', 's');
$email = dataFilter('email', 'e');
$pwd = dataFilter('pwd', 's');
$pwd_repeat = dataFilter('pwd_repeat', 's');


if (!filter_var($email, FILTER_VALIDATE_EMAIL)){
	die('2');
}

$sql = "SELECT email
		FROM korisnik
		WHERE email = :email
		AND id_korisnika != :id_korisnika;";
		
$stmt = $pdo -> prepare($sql);
$stmt -> bindValue(":email", $email, PDO::PARAM_STR);
$stmt -> bindValue(":id_korisnika", $id_korisnika, PDO::PARAM_STR);
$stmt -> execute();		
$result = $stmt -> fetchAll(PDO::FETCH_OBJ);

if ($stmt -> rowCount() != 0){
	die('4');
}


if (strlen($pwd) > 0 || strlen($pwd_repeat) > 0){
	if ($pwd !== $pwd_repeat){
		die('3');
	}
	
	$pwd_hash = password_hash($pwd, PASSWORD_DEFAULT, ['cost' => 12]);
	$sql = "UPDATE korisnik
			SET id_uloge=:id_uloge, 
				ime=:ime, 
				prezime=:prezime, 
				adresa=:adresa, 
				mjesto=:mjesto, 
				drzava=:drzava, 
				email=:email, 
				lozinka=:lozinka
			WHERE id_korisnika=:id_korisnika;";
		
	$stmt = $pdo -> prepare($sql);
	$stmt -> execute(array(':id_uloge'=>$uloga,
						   ':ime'=>$ime, 
						   ':prezime'=>$prezime, 
						   ':adresa'=>$adresa, 
						   ':mjesto'=>$mjesto, 
						   ':drzava'=>$drzava, 
						   ':email'=>$email, 
						   ':lozinka'=>$pwd_hash,
						   ':id_korisnika'=>$id_korisnika));
	
	if ($stmt -> rowCount() > 0){
		echo 6;
	}else {
		echo 5;
	}
}

if ($pwd == "" && $pwd_repeat == ""){
		
	$sql = "UPDATE korisnik
			SET id_uloge=:id_uloge, 
				ime=:ime, 
				prezime=:prezime, 
				adresa=:adresa, 
				mjesto=:mjesto, 
				drzava=:drzava, 
				email=:email
			WHERE id_korisnika=:id_korisnika;";
		
	$stmt = $pdo -> prepare($sql);
	$stmt -> execute(array(':id_uloge'=>$uloga,
						   ':ime'=>$ime, 
						   ':prezime'=>$prezime, 
						   ':adresa'=>$adresa, 
						   ':mjesto'=>$mjesto, 
						   ':drzava'=>$drzava, 
						   ':email'=>$email, 
						   ':id_korisnika'=>$id_korisnika));
	
	if ($stmt -> rowCount() > 0){
		echo 0;
	}
}


?>